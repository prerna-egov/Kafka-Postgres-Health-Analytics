...
CREATE UNIQUE INDEX idx_campaign_forecast_pk ON dm_campaign_forecast (campaign_id, product_name);
...
WITH district_deliveries AS (
    SELECT region_code, district_code, campaign_id, product_name, SUM(total_vaccinated) AS delivery_count
    FROM dm_successful_deliveries_base
...
SELECT
    ...
    RANK() OVER (
        PARTITION BY campaign_id, product_name
        ORDER BY actual_coverage DESC NULLS LAST
    ) AS coverage_rank
FROM district_metrics_computed;

CREATE UNIQUE INDEX idx_dist_perf_pk ON dm_district_performance (campaign_id, product_name, region_code, district_code);
...
-- KPI 7: District Performance Summary
SELECT district_code, region_code, delivery_count, target_population, actual_coverage, expected_coverage, coverage_rank FROM dm_district_performance WHERE campaign_id = :campaign_id ORDER BY coverage_rank;
