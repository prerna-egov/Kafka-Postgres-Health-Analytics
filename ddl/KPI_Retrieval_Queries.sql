-- ==============================================================================
-- CENTRALIZED KPI RETRIEVAL QUERIES
-- ==============================================================================

-- ////////////////////////////////////////////////////////////////
-- FROM: coverage_kpi_data_marts.sql
-- ////////////////////////////////////////////////////////////////
...
-- ////////////////////////////////////////////////////////////////
-- FROM: Registration_Data_Mart.sql
-- ////////////////////////////////////////////////////////////////
...
-- ---------------------------------------------------------------
SELECT region_code, SUM(enumerated_u5_count) AS total_children_enumerated FROM dm_registration_metrics_base WHERE country_code = :country_code AND campaign_id = :campaign_id GROUP BY region_code ORDER BY total_children_enumerated DESC;
...
-- Time Complexity: O(log N + K) (N = total rows, K = matching rows)
-- ---------------------------------------------------------------
SELECT region_code, SUM(total_households_registered) AS total_households_registered FROM dm_household_metrics_base WHERE country_code = :country_code AND campaign_id = :campaign_id GROUP BY region_code ORDER BY total_households_registered DESC;
...
-- Time Complexity: O(log N + K) (N = total rows, K = matching rows)
-- ---------------------------------------------------------------
SELECT region_code, age_band, SUM(enumerated_u5_count) AS children_count FROM dm_registration_metrics_base WHERE country_code = :country_code AND campaign_id = :campaign_id AND age_band != 'Other' GROUP BY region_code, age_band ORDER BY children_count DESC;
...
-- Time Complexity: O(log N + K) (N = total rows, K = matching rows)
-- ---------------------------------------------------------------
SELECT region_code, gender, SUM(enumerated_u5_count) AS children_count FROM dm_registration_metrics_base WHERE country_code = :country_code AND campaign_id = :campaign_id GROUP BY region_code, gender ORDER BY children_count DESC;
...
-- Time Complexity: O(log N + K) (N = total rows, K = matching rows)
-- ---------------------------------------------------------------
SELECT region_code, SUM(zero_dose_count) AS zero_dose_count FROM dm_registration_metrics_base WHERE country_code = :country_code AND campaign_id = :campaign_id GROUP BY region_code ORDER BY zero_dose_count DESC;
...
-- Time Complexity: O(log N + K) (N = total rows, K = matching rows)
-- ---------------------------------------------------------------
SELECT region_code, SUM(guest_member_count) AS guest_member_count FROM dm_registration_metrics_base WHERE country_code = :country_code AND campaign_id = :campaign_id GROUP BY region_code ORDER BY guest_member_count DESC;
...
-- Time Complexity: O(log N + K) (N = total rows, K = matching rows)
-- ---------------------------------------------------------------
SELECT campaign_id, SUM(failed_visit_count) AS failed_visit_count FROM dm_task_status_base WHERE country_code = :country_code AND campaign_id = :campaign_id GROUP BY campaign_id;
SELECT region_code, SUM(failed_visit_count) AS failed_visit_count FROM dm_task_status_base WHERE country_code = :country_code AND campaign_id = :campaign_id GROUP BY region_code ORDER BY failed_visit_count DESC;
...
-- Time Complexity: O(log N + K) (N = total rows, K = matching rows)
-- ---------------------------------------------------------------
SELECT campaign_id, SUM(is_refused) AS refused_beneficiaries, COUNT(beneficiary_id) AS total_beneficiaries, ROUND(SUM(is_refused) * 100.0 / NULLIF(COUNT(beneficiary_id), 0), 2) AS refusal_rate_pct FROM dm_beneficiary_status_base WHERE country_code = :country_code AND campaign_id = :campaign_id GROUP BY campaign_id;
SELECT region_code, SUM(is_refused) AS refused_beneficiaries, COUNT(beneficiary_id) AS total_beneficiaries, ROUND(SUM(is_refused) * 100.0 / NULLIF(COUNT(beneficiary_id), 0), 2) AS refusal_rate_pct FROM dm_beneficiary_status_base WHERE country_code = :country_code AND campaign_id = :campaign_id GROUP BY region_code ORDER BY refusal_rate_pct DESC;
...
-- Time Complexity: O(log N + K) (N = total rows, K = matching rows)
-- ---------------------------------------------------------------
SELECT campaign_id, SUM(is_absent) AS absent_beneficiaries, COUNT(beneficiary_id) AS total_beneficiaries, ROUND(SUM(is_absent) * 100.0 / NULLIF(COUNT(beneficiary_id), 0), 2) AS absence_rate_pct FROM dm_beneficiary_status_base WHERE country_code = :country_code AND campaign_id = :campaign_id GROUP BY campaign_id;
SELECT region_code, SUM(is_absent) AS absent_beneficiaries, COUNT(beneficiary_id) AS total_beneficiaries, ROUND(SUM(is_absent) * 100.0 / NULLIF(COUNT(beneficiary_id), 0), 2) AS absence_rate_pct FROM dm_beneficiary_status_base WHERE country_code = :country_code AND campaign_id = :campaign_id GROUP BY region_code ORDER BY absence_rate_pct DESC;
...
-- Time Complexity: O(log N + K) (N = total rows, K = matching rows)
-- ---------------------------------------------------------------
SELECT campaign_id, refusal_reason, SUM(refusal_count) AS refusal_count FROM dm_task_breakdown_base WHERE country_code = :country_code AND campaign_id = :campaign_id AND refusal_count > 0 GROUP BY campaign_id, refusal_reason ORDER BY refusal_count DESC;
SELECT region_code, refusal_reason, SUM(refusal_count) AS refusal_count FROM dm_task_breakdown_base WHERE country_code = :country_code AND campaign_id = :campaign_id AND refusal_count > 0 GROUP BY region_code, refusal_reason ORDER BY refusal_count DESC;
...
-- Time Complexity: O(log N + K) (N = total rows, K = matching rows)
-- ---------------------------------------------------------------
SELECT campaign_id, absence_category, SUM(absence_count) AS absence_count FROM dm_task_breakdown_base WHERE country_code = :country_code AND campaign_id = :campaign_id AND absence_count > 0 GROUP BY campaign_id, absence_category ORDER BY absence_count DESC;
SELECT region_code, absence_category, SUM(absence_count) AS absence_count FROM dm_task_breakdown_base WHERE country_code = :country_code AND campaign_id = :campaign_id AND absence_count > 0 GROUP BY region_code, absence_category ORDER BY absence_count DESC;
...
-- Time Complexity: O(log N + K) (N = total rows, K = matching rows)
-- ---------------------------------------------------------------
SELECT campaign_id, SUM(is_multi_unsuccessful) AS multi_unsuccessful_beneficiaries FROM dm_beneficiary_status_base WHERE country_code = :country_code AND campaign_id = :campaign_id GROUP BY campaign_id;
SELECT region_code, SUM(is_multi_unsuccessful) AS multi_unsuccessful_beneficiaries FROM dm_beneficiary_status_base WHERE country_code = :country_code AND campaign_id = :campaign_id GROUP BY region_code ORDER BY multi_unsuccessful_beneficiaries DESC;
SELECT district_code, SUM(is_multi_unsuccessful) AS multi_unsuccessful_beneficiaries FROM dm_beneficiary_status_base WHERE region_code = :region_code AND campaign_id = :campaign_id GROUP BY district_code ORDER BY multi_unsuccessful_beneficiaries DESC;
SELECT healthfacility_code, SUM(is_multi_unsuccessful) AS multi_unsuccessful_beneficiaries FROM dm_beneficiary_status_base WHERE district_code = :district_code AND campaign_id = :campaign_id GROUP BY healthfacility_code ORDER BY multi_unsuccessful_beneficiaries DESC;



-- ////////////////////////////////////////////////////////////////
-- FROM: Team_Performance_Mart.sql
-- ////////////////////////////////////////////////////////////////

-- #############################################################################
-- SECTION 3: KPI RETRIEVAL QUERIES
-- #############################################################################
...
SELECT region_code,
    ...
    SUM(under_1hr_count) AS under_1hr_count, SUM(one_to_6hr_count) AS one_to_6hr_count, SUM(six_to_24hr_count) AS six_to_24hr_count, SUM(over_24hr_count) AS over_24hr_count
FROM dm_team_sync_metrics_base
WHERE campaign_id = :campaign_id AND country_code = :country_code AND task_date = :task_date
...
-- ////////////////////////////////////////////////////////////////
-- FROM: Data_Quality_Marts.sql
-- ////////////////////////////////////////////////////////////////

-- ==============================================================================
-- 4. KPI RETRIEVAL QUERIES
-- ==============================================================================

4.1 Country Level Data Quality KPIs
...
4.2 Region Level Data Quality KPIs
...
4.3 District Level Data Quality KPIs
...
4.4 Health Facility Level Data Quality KPIs
...
4.5 Settlement Level Data Quality KPIs
